...
public class BookSearchFrame extends JInternalFrame {
	...
	private Object[][] getselect2(List list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			s[i][0] = book.getbookId();
			s[i][1] = book.getBookname();
			s[i][2] = book.getReaderId();
			s[i][3] =book.getReaderId();
			s[i][4] =book.getReaderName();
			s[i][5] =book.getReaderClass();
			s[i][6] = book.getPublisher();
			s[i][7]=book.getBorrowDate();
			s[i][8]=book.getBackDate();
			s[i][9]=book.getOperater();
		}
		return s;
	...
	public BookSearchFrame() {
		...
		panel_2_1.add(button);
		 ...
		 button.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					if(name.equals("图书名称")){				
						Object[][] results=getselect1(Dao.selectbookmohu(textField_1.getText()));
						table_2 = new JTable(results,booksearch);
						scrollPane_1.setViewportView(table_2);
					}
					else if(name.equals("图书作者")){
						
						Object[][] results=getselect1(Dao.selectbookmohuwriter(textField_1.getText()));
						table_2 = new JTable(results,booksearch);
		...
		Object[][] results=getselect2(Dao.selectbookserch());
		String [] booksearch = { "图书编号", "书名","学号","姓名","班级","作者", "出版社","借阅日期","归还日期","管理员编号"};
